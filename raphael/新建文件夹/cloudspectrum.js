...
CloudSpectrum.prototype = {
	...
	_init: function(){
		...
		if(cfg.mode == 'edit'){
			node.html('<div class="ui-cloudSpectrum" style="width:'+w+'px; height:'+h+'px; overflow:auto"><div id="'+divId+'"></div><div id="cp-preview-panel"></div><div id="cp-preview-btn">预览</div></div>');
			self.paper = Raphael(divId, w*3, h*2);
		}else{
			node.html('<div class="ui-cloudSpectrum" style="width:'+w+'px; height:'+h+'px; overflow:hidden"><div id="'+divId+'"></div></div>');
		...
		if(cfg.mode == 'edit'){
			var display = 1;
			var btn = self.dom().get('#cp-preview-btn');
			btn.bind('click', function(){
				var panel = self.dom().get('#cp-preview-panel');
				if(display){
					panel.show();
					btn.html('编辑');
					panel.html('');
	...
	render: function(data){
			...
			while(stack.length>0){
				...
				if(node){
					if(node.parent == 0){
						...
						shapes.push(r.set().push(drawHexagon(x, y, circleRadius*1.2).attr({'fill': node.color, 'stroke': node.color}).data('setIndex', idx), r.text(x, y, node.name).attr(TEXT_STYLE).data('setIndex', idx).data('dataIndex', idx)));
		...
		function drawSingleNode(node, pos, rad, angle){
			...
			var nodeShape;
			if(cfg.mode == 'edit'){
				nodeShape = r.circle(p[0], p[1], circleRadius).attr({'fill': node.color, 'stroke': node.color}).data('setIndex', idx);
			}else{
				switch(node.shape){
					case 1:
						nodeShape = r.rect(p[0]-circleRadius, p[1]-circleRadius, circleRadius*2, circleRadius*2).attr({'fill': node.color, 'stroke': node.color}).data('setIndex', idx);
					break;
					case 2:
						nodeShape = r.ellipse(p[0], p[1], circleRadius, circleRadius*0.6).attr({'fill': node.color, 'stroke': node.color}).data('setIndex', idx);
					break;
					case 3:
						nodeShape = r.circle(p[0], p[1], circleRadius).attr({'fill': node.color, 'stroke': node.color}).data('setIndex', idx);
					break;
					case 4:
						nodeShape = drawPentagonal(p[0], p[1], circleRadius).attr({'fill': node.color, 'stroke': node.color}).data('setIndex', idx);
					break;
					case 5:
						nodeShape = drawTriangle(p[0], p[1], circleRadius).attr({'fill': node.color, 'stroke': node.color}).data('setIndex', idx);
					break;
					case 6:
						nodeShape = drawHexagon(p[0], p[1], circleRadius).attr({'fill': node.color, 'stroke': node.color}).data('setIndex', idx);
					break;
					case 7:
						nodeShape = r.rect(p[0]-circleRadius, p[1]-circleRadius, circleRadius*2, circleRadius*2, 10).attr({'fill': node.color, 'stroke': node.color}).data('setIndex', idx);
					break;
				}
			}
